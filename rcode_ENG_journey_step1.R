...
journey_nl <- read.csv2("journey_eng_hash_2.csv")
...
resultaten$aantal_bezoeken <- gsub("\\D", "", resultaten$aantal_bezoeken)

write.csv(resultaten, file = "ticket_eng_clean.csv", row.names = FALSE)
...
resultaten <- resultaten %>%
  mutate(aantal_bezoeken = as.numeric(aantal_bezoeken))

resultaten <- resultaten %>%
  mutate(aantal_bezoeken = ifelse(aantal_bezoeken == "na", 1, aantal_bezoeken))
